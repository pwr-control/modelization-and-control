...
t1c = time_tc_sim(end) - Nc*tc/400;
t2c = time_tc_sim(end);
N1c = floor(Nc/400);
N2c = floor(Nc);

t3c = time_tc_sim(end) - Nc*tc/2000;
t4c = time_tc_sim(end);
N3c = floor(Nc/2000);
N4c = floor(Nc);

t1s = time_tc_sim(end) - (Nc*tc/10);
t2s = time_tc_sim(end);
N1s = floor(Nc/10);
N2s = floor(Nc);
...
grid on
subplot 212
plot(time_tc_sim,inverter_voltage_output_sim,'-','LineWidth',tratto3,'Color',colore1);
...
plot(ax2,x2,y2,'-','LineWidth',tratto1,'Color',colore1);
grid on
