clear;
[model, options] = init_environment('single_phase_dab');
%[text] ### Global timing
% simulation length
simlength = 1.25;
...
hwdata.afe = three_phase_afe_hwdata(application_voltage, afe_pwr_nom, fPWM_AFE); %[output:07196b42]
hwdata.inv = three_phase_inverter_hwdata(application_voltage, inv_pwr_nom, fPWM_INV); %[output:20452e44]
hwdata.dab = single_phase_dab_hwdata(application_voltage, dab_pwr_nom, fPWM_DAB, fres_dab); %[output:1c8baa00]
hwdata.cllc = single_phase_cllc_hwdata(application_voltage, dab_pwr_nom, fPWM_CLLC, fres_cllc); %[output:1aec92b0]
...
l1 = Kd(2) %[output:9ff4dd1f]
l2 = Kd(1) %[output:62ebdd04]
...
parasitic_dclink_data; %[output:5c304d9d]
...
Ldrso = acker(Adrso',Crso',polesdrso)' %[output:3f453bd5]
...
delta_fht0 = 0.05;
Afht0 = [0 1; -omega_fht0^2 -delta_fht0*omega_fht0] % impianto nel continuo %[output:94224c9e]
Cfht0 = [1 0];
poles_fht0 = [-1 -4]*omega_fht0;
Lfht0 = acker(Afht0',Cfht0', poles_fht0)' % guadagni osservatore nel continuo %[output:3b717727]
Ad_fht0 = eye(2) + Afht0*ts_afe % impianto nel discreto %[output:88b0dea4]
polesd_fht0 = exp(ts_afe*poles_fht0);
Ld_fht0 = acker(Ad_fht0',Cfht0', polesd_fht0) %[output:3a43cfed]

%[text] ### First Harmonic Tracker for Load
omega_fht1 = grid_emu_data.w_grid;
delta_fht1 = 0.05;
Afht1 = [0 1; -omega_fht1^2 -delta_fht1*omega_fht1] % impianto nel continuo %[output:0671c553]
Cfht1 = [1 0];
poles_fht1 = [-1 -4]*omega_fht1;
Lfht1 = acker(Afht1', Cfht1', poles_fht1)' % guadagni osservatore nel continuo %[output:463911a0]
Ad_fht1 = eye(2) + Afht1*ts_afe % impianto nel discreto %[output:478d863c]
polesd_fht1 = exp(ts_afe*poles_fht1);
Ld_fht1 = acker(Ad_fht1',Cfht1', polesd_fht1) %[output:8dea43cc]
...
n_sys = 6;
run('n_sys_generic_1M5W_pmsm'); %[output:46b0e455] %[output:45030ab5]
...
kg = Kobs(1) %[output:91f41228]
kw = Kobs(2) %[output:2268b47c]
...
luenberger_l1 = Klo(1) %[output:9ea83ef9]
luenberger_l2 = Klo(2) %[output:38336db4]
luenberger_l3 = Klo(3) %[output:3b7ec421]
omega_flt_fcut = 10;
...
lithium_ion_battery_1 = lithium_ion_battery(nominal_battery_voltage, nominal_battery_power, initial_battery_soc, ts_dab); %[output:19da337d]
lithium_ion_battery_2 = lithium_ion_battery(nominal_battery_voltage, nominal_battery_power, initial_battery_soc, ts_dab); %[output:459f1103]
