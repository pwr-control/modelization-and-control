...
deepPOSxi = 0.5 %[output:5addcc28]
deepNEGxi = 0 %[output:92d64065]
deepNEGeta = 0.5 %[output:62c37f0f]
%[text] #### 
...
l1 = Kd(2) %[output:0670012e]
l2 = Kd(1) %[output:73d52bd1]
...
Ldrso = acker(Adrso',Crso',polesdrso)' %[output:273b8593]
...
omega0 = 2*pi*50;
Afht = [0 1; -omega0^2 -0.05*omega0] % impianto nel continuo %[output:47d57e79]
Cfht = [1 0];
poles_fht = [-1 -4]*omega0;
Lfht = acker(Afht',Cfht',poles_fht)' % guadagni osservatore nel continuo %[output:85380659]
Ad_fht = eye(2) + Afht*ts_afe % impianto nel discreto %[output:954fcb8d]
polesd_fht = exp(ts_afe*poles_fht);
Ld_fht = Lfht*ts_afe % guadagni osservatore nel discreto %[output:1298f239]
...
run('loopback_250kW'); %[output:61dbd81c]
run('loopback_torque_curve');
...
kg = Kobs(1) %[output:1d1cc601]
kw = Kobs(2) %[output:1bf80c18]
...
luenberger_l1 = Klo(1) %[output:9d31fb1d]
luenberger_l2 = Klo(2) %[output:16271dfb]
luenberger_l3 = Klo(3) %[output:2e701db9]
omega_flt_fcut = 10;
...
heatsink_liquid_2kW; %[output:7167fec9] %[output:3a2a034a] %[output:69d9cd6f]
...
%---
%[output:61dbd81c]
%   data: {"dataType":"textualVariable","outputData":{"name":"vg_dclink","value":"     5.382628063277161e+02"}}
%---
%[output:1d1cc601]
...
%---
%[output:2e701db9]
%   data: {"dataType":"textualVariable","outputData":{"name":"luenberger_l3","value":"    -9.233649128079331e+06"}}
%---
%[output:7167fec9]
